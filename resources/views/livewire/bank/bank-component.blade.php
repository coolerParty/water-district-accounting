<div>
    ...
    <div
        ...
        <h1 class="text-2xl font-semibold whitespace-nowrap">Bank</h1>
        @can('user-create')
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="transition-all hover:bg-gray-100 hover:shadow-lg">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $bank->seq_no }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $bank->bank_name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $bank->desc_name_brs }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $bank->acct_number }}</div>
                                </td>
                                <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                    @can('user-edit')
                                    <x-link-success href="{{ route('bank.edit', ['id' => $bank->id]) }}"> Edit
                                    </x-link-success>
                                    @can('user-delete')
                                    @endcan
                                    <x-link-danger href="#" class="btn btn-danger btn-sm text-light"
                                        onclick="confirm('Are you sure, You want to delete this bank record?') || event.stopImmediatePropagation()"
                                        wire:click.prevent="destroy({{ $bank->id }})">
                                        Delete
                                    </x-link-danger>
                                    @endcan
                                </td>
